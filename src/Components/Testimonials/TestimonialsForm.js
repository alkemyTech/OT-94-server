...
const TestimonialForm = () => {
    ...
    const handleSubmit = (e) => {
        ...
        if (validation.name.test(initialValues.name) && validation.image.test(initialValues.image) && !validation.description.test(initialValues.description)) {
            if (petition === true) {
                axios.post('/testimonials/create', initialValues)
                .then((response) => response)
                .catch((error) => error)
            } else {
                axios.patch('/testimonials/:id', initialValues)
                .then ((response) => response)
                .catch((error) => error)
            }
        } else {
    ...
    return (
        <form className="form-container" onSubmit={handleSubmit}>
            ...
            {errorName ? <p>Error, it must have a minimum length of 4 characters</p> : null}
